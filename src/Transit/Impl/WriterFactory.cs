...
namespace Beerendonk.Transit.Impl
{
    ...
    internal partial class WriterFactory
    {
            ...
            IWriteHandler defaultHandler, Func<object, object> transform)
        {
            ...
            if (verboseMode)
            {
                emitter = new Sellars.Transit.Cljr.Impl.Utf8JsonVerboseEmitter(jsonWriter, GetVerboseHandlers(handlers), defaultHandler, transform);
            }
            else
            {
                emitter = new Sellars.Transit.Cljr.Impl.Utf8JsonEmitter(jsonWriter, handlers, defaultHandler, transform);
            }
